...
const chartOptions = reactive<Record<string, EChartsOption>>({
  ...
  connections: getMultiSeriesOption(
    "网络连接数",
    [
      {
        type: "line",
  ...
  // 网络分类 - 总网卡 IO
  network_total: getMultiSeriesOption(
    "总网卡 IO",
    [
      {
        type: "line",
  ...
  // 网络分类 - pppoe-wan IO
  network_pppoe_wan: getMultiSeriesOption(
    "pppoe-wan IO",
    [
      {
        type: "line",
...
const getCpuCoreCharts = computed(() => {
  return Object.entries(chartOptions).filter(([key]) => key.startsWith("cpu_core_"));
});
...
const getNetworkCharts = computed(() => {
  return Object.entries(chartOptions).filter(([key]) => key === "network_iface_all");
});

const getStorageCharts = computed(() => {
  return Object.entries(chartOptions).filter(
    ([key]) => key.startsWith("storage_io_") || key.startsWith("storage_space_"),
  );
});
...
const loadHistoryAndRenderMultiSeries = async (
  chartKey: string,
  metric: string,
  seriesIndex: number,
  label: string,
  ...
  if (data.storage) {
    Object.keys(data.storage).forEach((key) => {
      ...
      if (!chartOptions[chartKeySpace]) {
        ...
        chartOptions[chartKeySpace] = getFixedAxisOption(
          `${key} 存储空间`,
          "#06b6d4",
          storageData.total.unit,
          0,
          maxValue,
        );
...
const appendDataPointMultiSeries = (
  chartKey: string,
  seriesIndex: number,
  timestamp: number,
  value: number,
  ...
  const seriesArr = (chartOptions[chartKey].series as { data: [number, number][] })[seriesIndex]
    .data;
...
watch(
  ...
  (newData) => {
    ...
    if (cpuUsage?.value !== undefined)
      appendDataPoint("cpu_total", now, cpuUsage.value, cpuUsage.unit);

    // 基本指标 - CPU Temp
    if (newData.cpu) {
      let totalTemp = 0,
        count = 0;
      Object.values(newData.cpu).forEach((c: any) => {
    ...
    if (memUsage?.value !== undefined)
      appendDataPoint("memory_used_percent", now, memUsage.value, memUsage.unit);
...
const handleGlobalRangeChange = async (event: Event) => {
  ...
  Object.keys(chartOptions).forEach((key) => {
    if (key.startsWith("storage_io_")) {
      ...
      !["connections", "network_total", "network_pppoe_wan", "network_iface_all"].includes(key)
    ) {
...
onMounted(async () => {
  ...
  // 加载CPU核心数据
  Object.keys(chartOptions)
    .filter((k) => k.startsWith("cpu_core_"))
    .forEach((k) => loadHistoryAndRender(k));
  ...
  // 加载存储IO和空间
  Object.keys(chartOptions)
    .filter((k) => k.startsWith("storage_io_"))
    .forEach((k) => {
      const storageKey = k.replace("storage_io_", "");
      loadStorageIoHistory(storageKey);
    });
  Object.keys(chartOptions)
    .filter((k) => k.startsWith("storage_space_"))
    .forEach((k) => {
