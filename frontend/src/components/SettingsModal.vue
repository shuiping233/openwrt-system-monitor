...
<template>
  ...
  >
    <div
      v-if="isOpen"
      class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-none"
    ...
    >
      ...
      >
        ...
        <div class="px-6 py-6 space-y-6 overflow-y-auto">
          ...
          <div>
            ...
            <div class="mt-4 space-y-4">
              ...
              <div class="flex justify-between items-center">
                <label
                  class="text-sm"
                  :class="settings.enable_metric_record ? 'text-slate-300' : 'text-slate-500'"
                >
                  数据保留天数
                </label>
                <input
                  ...
                  class="border rounded px-3 py-1.5 w-24 outline-none transition-colors"
                  :class="
                    settings.enable_metric_record
                      ? 'bg-slate-900 border-slate-600 text-white focus:border-blue-500'
                      : 'bg-slate-800 border-slate-700 text-slate-500 cursor-not-allowed'
                  "
                />
              </div>
              <p class="text-xs text-slate-500">超过此天数的历史图表数据将被自动清理。</p>
          ...
          <div>
            ...
            <div class="mt-4 space-y-4">
              ...
              <p class="text-xs text-slate-500">DNS 查询结果的缓存时间，过期后将重新查询。</p>
              ...
              <p class="text-xs text-slate-500">每次 DNS 查询请求携带的最大 IP 地址数量。</p>
              ...
              <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-2">
                  <input
                    ...
                    min="5"
                    max="300"
                    :value="settings.dns_poll_interval"
                    @change="
                      (e: Event) =>
                        handleSave(
                          'dns_poll_interval',
          ...
          <div>
            ...
            <div class="mt-4 space-y-4">
              ...
              <div class="flex justify-between items-center">
                <label
                  class="text-sm"
                  :class="settings.enable_background_stop ? 'text-slate-300' : 'text-slate-500'"
                >
                  后台停止延迟
                </label>
                <div class="flex items-center gap-2">
                  <input
                    ...
                    min="10"
                    max="600"
                    :value="settings.background_stop_delay"
                    @change="
                      (e: Event) =>
                        handleSave(
                          'background_stop_delay',
                          parseInt((e.target as HTMLInputElement).value, 10),
                        )
                    "
                    :disabled="!settings.enable_background_stop"
                    class="border rounded px-3 py-1.5 w-20 outline-none transition-colors"
                    :class="
                      settings.enable_background_stop
                        ? 'bg-slate-900 border-slate-600 text-white focus:border-blue-500'
                        : 'bg-slate-800 border-slate-700 text-slate-500 cursor-not-allowed'
                    "
                  />
        ...
        <!-- 底部: 退出按钮 (右下角) -->
        <div
          class="px-6 py-4 border-t border-slate-700 flex justify-end bg-slate-800/50 rounded-b-xl shrink-0"
        >
          <button
            @click="emit('update:isOpen', false)"
