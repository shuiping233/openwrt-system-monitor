      ...
      <div v-if="data.dynamic.memory?.used_percent"
        ...
        <div class="flex justify-between items-baseline mt-1">
          ...
          <div class="text-xl font-bold">
            {{
              formatMetric(
                data.dynamic.memory.used_percent.value,
                data.dynamic.memory.used_percent.unit,
              )
            }}
          </div>
        ...
        <div class="flex flex-col">
          ...
          <div v-if="data.dynamic.network?.['pppoe-wan']?.outgoing" class="flex items-center justify-between">
            ...
            <div class="text-xl font-bold font-mono text-orange-500">
              {{
                formatMetric(
                  data.dynamic.network["pppoe-wan"].outgoing.value,
          ...
          <div v-if="data.dynamic.network?.['pppoe-wan']?.incoming" class="flex items-center justify-between mb-1">
            ...
            <div class="text-xl font-bold font-mono text-cyan-500">
              {{
                formatMetric(
                  data.dynamic.network["pppoe-wan"].incoming.value,
      ...
      " class="bg-slate-800 border border-slate-700 rounded-xl p-5">
        ...
        <div v-if="data.dynamic.network?.['pppoe-wan']?.outgoing" class="flex items-center justify-between">
          ...
          <div class="text-xl font-bold font-mono text-orange-500">
            {{
              formatMetric(
                data.dynamic.network.total.incoming.value,
        ...
        <div v-if="data.dynamic.network?.['pppoe-wan']?.incoming" class="flex items-center justify-between mb-1">
          ...
          <div class="text-xl font-bold font-mono text-cyan-500">
            {{
              formatMetric(
                data.dynamic.network.total.outgoing.value,
      ...
      <div v-show="uiState.accordions.storage" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        <div v-for="(dev, name) in data.dynamic.storage" :key="name"
          ...
          <div class="grid grid-cols-3 gap-2 text-sm mb-3">
            <div>
              <span class="text-slate-500">读:</span>
              {{ formatValue(dev.read.value, dev.read.unit) }}
              <span class="font-mono text-slate-200"> {{ dev.read.unit }}</span>
            </div>
            <div>
              <span class="text-slate-500">写:</span>
              {{ formatValue(dev.write.value, dev.write.unit) }}
              <span class="font-mono text-slate-200"> {{ dev.write.unit }}</span>
            </div>
            ...
            <div>
              ...
              <span class="font-mono">{{ formatMetric(dev.total.value, dev.total.unit) }}</span>
            </div>
            <div>
              <span class="text-slate-500">已用:</span>
              <span class="font-mono">{{ formatMetric(dev.used.value, dev.used.unit) }}</span>
            </div>
          </div>
      ...
      <div v-show="uiState.accordions.network" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <template v-for="(net, iface) in data.dynamic.network" :key="'io-' + iface">
          <div v-if="iface !== 'total'"
            ...
            <div class="flex justify-between items-center">
              <div class="text-xl font-bold font-mono text-cyan-500">
                ↓ {{ formatMetric(net.incoming.value, net.incoming.unit) }}
              </div>
              <div class="text-xl font-bold font-mono text-orange-500">
                ↑ {{ formatMetric(net.outgoing.value, net.outgoing.unit) }}
              </div>
            </div>
          </div>
        </template>
      ...
      <div v-show="uiState.accordions.sysinfo" class="bg-slate-800 border border-slate-700 rounded-xl p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6">
          <div>
            ...
            <span class="font-medium">{{ data.static.system.os }}</span>
          </div>
          <div>
            <span class="text-slate-500 block text-sm mb-1">Hostname:</span>
            <span class="font-medium">{{ data.static.system.hostname }}</span>
          </div>
          <div>
            <span class="text-slate-500 block text-sm mb-1">Kernel:</span>
            <span class="font-medium">{{ data.static.system.kernel }}</span>
          </div>
          <div>
            <span class="text-slate-500 block text-sm mb-1">Device:</span>
            <span class="font-medium">{{ data.static.system.device_name }}</span>
          </div>
          <div>
            <span class="text-slate-500 block text-sm mb-1">Arch:</span>
            <span class="font-medium">{{ data.static.system.arch }}</span>
          </div>
          <div>
            <span class="text-slate-500 block text-sm mb-1">Timezone:</span>
            <span class="font-medium">{{ data.static.system.timezone }}</span>
          </div>
        </div>
      </div>
    </div>
